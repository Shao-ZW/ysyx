...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", beq    , B, (src1 == src2) ? (s->dnpc = s->pc + imm) : (s->dnpc = s->dnpc));
  INSTPAT("??????? ????? ????? 001 ????? 11001 11", bne    , B, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 100 ????? 11001 11", blt    , B, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 101 ????? 11001 11", bge    , B, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 110 ????? 11001 11", bltu   , B, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 111 ????? 11001 11", bgeu   , B, R(rd) = Mr(src1 + imm, 1));
