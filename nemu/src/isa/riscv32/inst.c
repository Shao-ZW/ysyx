...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", beq    , B, ((src1 == src2) ? (s->dnpc = s->pc + imm) : 0));
  INSTPAT("??????? ????? ????? 001 ????? 11001 11", bne    , B, (src1 != src2) ? (s->dnpc = s->pc + imm) : 0);
  INSTPAT("??????? ????? ????? 100 ????? 11001 11", blt    , B, ((sword_t)src1 < (sword_t)src2) ? (s->dnpc = s->pc + imm) : 0);
  INSTPAT("??????? ????? ????? 101 ????? 11001 11", bge    , B, ((sword_t)src1 > (sword_t)src2) ? (s->dnpc = s->pc + imm) : 0);
  INSTPAT("??????? ????? ????? 110 ????? 11001 11", bltu   , B, (src1 < src2) ? (s->dnpc = s->pc + imm) : 0);
  INSTPAT("??????? ????? ????? 111 ????? 11001 11", bgeu   , B, (src1 > src2) ? (s->dnpc = s->pc + imm) : 0);
